...
| `1` – `7` (with Inspector focused) | Switch between sub-tabs (Files, Git, Search, Tasks, History, Notes, Memory) |

Focus the Inspector by clicking inside it once, then the digit
keys take effect. ESC returns focus to the terminal. See the
[Inspector panel](#02-sessions-03-inspector) page for what each
sub-tab does.
